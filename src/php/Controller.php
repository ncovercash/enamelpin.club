...
use EnamelPinClub\API\{Endpoint, Response};
use EnamelPinClub\Database\{AbstractDatabaseModel, AbstractDatabaseRowModel};
use EnamelPinClub\HTTPCode;
use EnamelPinClub\Images\{DBImage, Image};
use \Exception;
use \LogicException;
use \PDO;
use \ReflectionClass;
...
class Controller {
	...
	public static function loadClass(string $name): void {
		if (strpos($name, "EnamelPinClub\\") === 0) {
			$unNamespaced = str_replace("EnamelPinClub\\", "", $name);
	...
	/**
	 * Sends an discord webhook reporting the error
	 *
	 ...
	 */
	public static function sendErrorNotice(
		string $subj,
		string $errco,
		int $errno,
		string $errstr,
		...
		if (php_sapi_name() == "cli") {
			...
			$ua = [
				new Parser(getallheaders()),
				new Parser(getallheaders(), ["detectBots" => false]),
			];
		...
		file_get_contents(
			"https://discordapp.com/api/webhooks/" . Secrets::get("DISCORD_BUG_WEBHOOK_TOKEN"),
			false,
								...
								"fields" => array_merge(
									[
										...
										[
											...
											"value" => array_key_exists("PHP_SELF", $_SERVER)
												? $_SERVER["PHP_SELF"]
												: "unknown",
										],
										[
											"name" => "Referrer",
											"value" => array_key_exists("HTTP_REFERER", $_SERVER)
	 ...
	 */
	public static function send500Error(
		int $errno,
		string $errstr,
		string $errfile,
		int $errline,
		...
		if (php_sapi_name() === "cli") {
			echo "An unexpected error occurred!  Information:\n" .
				"  ERRNO:      " .
				$errno .
				"\n" .
				"  ERRSTR:     " .
		...
		if (Endpoint::isEndpoint()) {
			if (!headers_sent()) {
				...
				trigger_error(
					"API ENDPOINT HEADERS (SOMEHOW) ALREADY SENT BEFORE ERROR!",
					E_USER_NOTICE,
				);
			}
		} else {
			if (headers_sent()) {
				// assume we already sent header, despite our OB attempts
				echo '</div><div a=""></div></div></div></p></img></div></div><h3>An unexpected error occurred.  Tracking ID: ' .
					$trackingId .
					"</h3></body></html>";
			} else {
				if (!defined("REAL_ROOTDIR")) {
					define("REAL_ROOTDIR", __DIR__ . "/../,,."); // we can do this safely because we're only using stuff from this script
				}

				require REAL_ROOTDIR . "src/html/head.inc.php";

				echo '<div class="section center row">';
				echo "<h3>An unexpected error occurred</h3>";
				echo '<p class="flow-text no-bottom-margin">Tracking ID: <strong>' .
					$trackingId .
					"</strong></p>";
				echo '<p class="flow-text no-top-margin">If you would please e-mail <a href="mailto:webmaster@enamelpin.club">webmaster@enamelpin.club</a> with the above tracking ID, what you were doing, and any other relevant information.</p>';
				...
				if (self::isDevelMode()) {
					...
					echo "<p>SECRETS DEFINED: " .
						(class_exists(Secrets::class) ? "YES" : "NO") .
						"</p>";
					...
					echo "<p>MySQL PDO: " .
						(in_array("mysql", PDO::getAvailableDrivers(), true) ? "YES" : "NO") .
						"</p>";
					echo "</div>";
					...
					echo "<p>Cookies: " .
						htmlspecialchars(
							array_key_exists("HTTP_COOKIE", $_SERVER)
								? $_SERVER["HTTP_COOKIE"]
								: "unknown",
						) .
						"</p>";
					...
					echo "<p>Remote: " .
						...
						":" .
						htmlspecialchars(
							array_key_exists("REMOTE_PORT", $_SERVER)
								? $_SERVER["REMOTE_PORT"]
								: "unknown",
						) .
						"</p>";
					echo "<p>argv: " .
						htmlspecialchars(
							array_key_exists("ARGV", $_SERVER) ? $_SERVER["ARGV"] : "unknown",
						) .
						"</p>";
					echo "<p></p>";
					...
					echo "<p>Version: " .
						htmlspecialchars(
							Controller::getVersion() . " (" . self::getCommit() . ")",
						) .
						"</p>";
					echo "</div>";
				}
				...
				require REAL_ROOTDIR . "src/html/footer.inc.php";
			}
			ob_flush();
			flush();
			die();
	...
	public static function getCanonicalRequestUrl(): string {
		...
		return self::getRequestUrl() .
			(strpos(strrev(self::getRequestUrl()), "/") !== 0 ? "/" : "");
	}
