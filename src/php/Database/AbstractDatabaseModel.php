...
use EnamelPinClub\Database\Query\{InsertQuery, SelectQuery, UpdateQuery};
use EnamelPinClub\Database\QueryAddition\WhereClause;
...
abstract class AbstractDatabaseModel implements Serializable {
	 ...
	 *
	 * @param int $id
	 * @param mixed[] $cache Prefilled cache
	...
	public function __construct(int $id, array $cache = [], bool $prefetch = true) {
			...
			if ($prefetch) {
				...
				if ($row === false) {
					throw new InvalidArgumentException(
						"ID " . $id . " does not exist in table " . static::getTable() . ".",
					);
				}
		...
		self::$cache[static::class][$this->id] = array_merge(
			self::$cache[static::class][$this->id],
			$cache,
		);
	}
	...
	protected function getColumnFromDatabaseOrCache(string $column) {
		...
		return self::$cache[static::class][$this->id][$column] = $this->getColumnFromDatabase(
			$column,
		);
	}
	...
	protected function getDataFromCallableOrCache(string $key, callable $callable) {
		...
		return self::$cache[static::class][$this->id][$key] = Closure::fromCallable(
			$callable,
		)->bindTo($this)();
	}
	 ...
	 * @return mixed
	 */
	protected static function getStaticDataFromCallableOrCache(string $key, callable $callable) {
		if (!array_key_exists(static::class, self::$cache)) {
			self::$cache[static::class] = [];
		}
		if (!array_key_exists("_static", self::$cache[static::class])) {
			self::$cache[static::class]["_static"] = [];
		}
		if (array_key_exists($key, self::$cache[static::class]["_static"])) {
			return self::$cache[static::class]["_static"][$key];
		}
		return self::$cache[static::class]["_static"][$key] = Closure::fromCallable($callable)();
	}
	...
	/**
	 * Remove a selected object type from the internal cache completely
	 */
	public static function _clearAllOfCacheType(): void {
	 ...
	 */
	protected function updateColumnInDatabase(
		string $column,
		$value,
	...
	public function unserialize($data): void {
		...
		if ($row === false) {
			throw new InvalidArgumentException(
				"ID " . $this->id . " does not exist in table " . static::getTable() . ".",
			);
		}
	 ...
	 */
	protected function unserializeVerification(): void {
	}

	/**
	 * Create a new object from the given info
	 *
	 * @param mixed[] $values
	 * @return static
	 */
	public static function create(array $values) {
		// no way to type-hint a subclass
		...
		return new static($stmt->getResult(), $values, false);
	}
	 ...
	 * @return mixed[]
	 */
	abstract protected static function getModifiableProperties(): array;
	...
	public function __call(string $name, array $arguments) {
		...
		if ($type == "get" || $type == "is") {
			if (is_null($methodDefinition[1])) {
				...
				return $this->getDataFromCallableOrCache(
					"_custom_hander_" . $methodDefinition[0],
					function () use ($type, $methodDefinition) {
						if ($type == "is") {
							return (bool) call_user_func(
								$methodDefinition[1],
								$this->getColumnFromDatabaseOrCache($methodDefinition[0]),
							);
						}
						if (
			...
			if (is_null($methodDefinition[2])) {
				...
				if (
					$this->getColumnFromDatabaseOrCache($methodDefinition[0]) ==
					call_user_func($methodDefinition[2], $arguments[0])
				) {
					return;
				}
				$this->updateColumnInDatabase(
					$methodDefinition[0],
					call_user_func($methodDefinition[2], $arguments[0]),
				);
			}
