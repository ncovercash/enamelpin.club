...
class WhereClause implements QueryAdditionInterface {
	 ...
	 *
	 * @param mixed[] $clause Items for the clause.  [column, equality, value] delimited by WhereClause::AND or WhereClause::OR
	 */
	 ...
	 *
	 * @return mixed[] Items for the clause.  [column, equality, value] delimited by WhereClause::AND or WhereClause::OR
	 */
		...
		foreach ($this->clause as $value) {
			if (!is_array($value)) {
				switch ($value) {
					...
					default:
						throw new LogicException(
							"Where clause item has no associated column and is not equal to WhereClause::AND or WhereClause::OR",
						);
				}
			} else {
				if (count($value) != 3) {
				...
				if (!$value[0] instanceof Column) {
					throw new InvalidArgumentException(
						"Column expected in WhereClause, found " .
							(gettype($value[0]) == "object"
								? get_class($value[0])
								: gettype($value[0])),
					);
				}
