...
use EnamelPinClub\Database\Column;
use EnamelPinClub\Database\QueryAddition\QueryAdditionInterface;
...
abstract class AbstractQuery {
	 ...
	 * @var int
	 */
	protected static $totalQueries = 0;
	 ...
	 * @param mixed[] $values List of values to bind to the above columns
	...
	public function removeColumn(Column $column): bool {
		...
		$this->columns = array_filter($this->columns, function (Column $in) use ($column) {
			return $in !== $column;
		});
	...
	public function removeColumns(array $columns): bool {
		$initialCount = count($this->columns);
		$this->columns = array_filter($this->columns, function (Column $in) use ($columns) {
	 ...
	 * @param mixed[] $values List of values to add to the value array
	 */
	 ...
	 *
	 * $values must be an array in the order of values to bind
	 * @param mixed[] $values New list of values
	 */
		...
		foreach ($this->additionalCapabilities as $additionalCapability) {
			if (!($additionalCapability instanceof QueryAdditionInterface)) {
				throw new InvalidArgumentException(
					"Additional capability is not a QueryAdditionInterface",
				);
			}
		}
		return true;
	 ...
	 *
	 * @return mixed[] Parameters to pass to PDOStatement->execute
	 */
